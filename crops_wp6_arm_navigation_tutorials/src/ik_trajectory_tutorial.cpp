...
#include <crops_wp6_arm_navigation_tutorials/FollowJointTrajectoryControllerState.h>
#include <actionlib/client/simple_action_client.h>
#include <kinematics_msgs/GetPositionIK.h>
#include <kinematics_msgs/GetPositionFK.h>
#include <crops_wp6_arm_navigation_tutorials/ExecuteCartesianIKTrajectory.h>
#include <vector>
...
static const std::string ARM_IK_NAME = "/crops_wp6_arm_kinematics/get_ik";
typedef actionlib::SimpleActionClient<control_msgs::
...
public:
  IKTrajectoryExecutor(){
    ...
    action_client = new TrajClient("crops_arm_controller/follow_joint_trajectory", true);

    //wait for the action server to come up
    while(ros::ok() && !action_client->waitForServer(ros::Duration(5.0))){
      ROS_INFO("Waiting for the crops_arm_controller/follow_joint_trajectory action server to come up");
    }
  ...
  bool run_ik(geometry_msgs::PoseStamped pose, double start_angles[9], 
              double solution[9], std::string link_name){

    kinematics_msgs::GetPositionIK::Request  ik_request;
    kinematics_msgs::GetPositionIK::Response ik_response;  
 
    ik_request.timeout = ros::Duration(5.0);
ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint1");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint2");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint3");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint4");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint5");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint6");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint7");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint8");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("joint9");

    ik_request.ik_request.ik_link_name = link_name; 

    ik_request.ik_request.pose_stamped = pose;
    ik_request.ik_request.ik_seed_state.joint_state.position.resize(9);

    for(int i=0; i<9; i++) ik_request.ik_request.ik_seed_state.joint_state.position[i] = start_angles[i];
  ...
  void get_current_joint_angles(double current_angles[9]){
    int i;

    //get a single message from the topic 'r_arm_controller/state'
    crops_wp6_arm_navigation_tutorials::FollowJointTrajectoryControllerStateConstPtr state_msg = 
      ros::topic::waitForMessage<crops_wp6_arm_navigation_tutorials::FollowJointTrajectoryControllerState>
      ("crops_arm_controller/state");
  ...
  bool execute_joint_trajectory(std::vector<double *> joint_trajectory){
    ...
    goal.trajectory.points[0].positions.resize(9);
    goal.trajectory.points[0].velocities.resize(9);
    for(j=0; j<9; j++){
  ...
  bool execute_cartesian_ik_trajectory(
         crops_wp6_arm_navigation_tutorials::ExecuteCartesianIKTrajectory::Request &req,
         crops_wp6_arm_navigation_tutorials::ExecuteCartesianIKTrajectory::Response &res){
    ...
    for(i=0; i<trajectory_length; i++){
      ...
      double *trajectory_point = new double[9];
      success = run_ik(stamped_pose, last_angles, trajectory_point, "body9");
      ...
      for(j=0; j<9; j++) last_angles[j] = trajectory_point[j];
    }        
